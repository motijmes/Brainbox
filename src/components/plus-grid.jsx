...
export function PlusGridRow({ className = '', children, isHome }) {
  return (
    <div
      className={clsx(
        className,
    ...
    >
      ...
      >
        <div
          className={`absolute inset-x-0 top-0 ${isHome ? 'h-px bg-gradient-to-r from-transparent via-white/10 to-transparent' : 'border-t border-black/5'}`}
        ></div>
        <div
          className={`absolute inset-x-0 top-2 ${isHome ? 'h-px bg-gradient-to-r from-transparent via-white/10 to-transparent' : 'border-t border-black/5'}`}
        ></div>
        <div
          className={`absolute inset-x-0 bottom-0 hidden ${isHome ? 'h-px bg-gradient-to-r from-transparent via-white/10 to-transparent group-last/row:block' : 'border-b border-black/5 group-last/row:block'}`}
        ></div>
        <div
